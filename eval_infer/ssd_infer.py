...
# device = torch.device("cuda:%d"%config.device_id if torch.cuda.is_available() else "cpu")
# net = net.to(device)
if net_type == 'vgg16_ssd':
    predictor = create_vgg_ssd_predictor(net, candidate_size=200,device_id=config.device_id)
elif net_type == 'mb1_ssd':
    predictor = create_mobilenetv1_ssd_predictor(net, candidate_size=200,device_id=config.device_id)
elif net_type == 'mb1_ssd_lite':
    predictor = create_mobilenetv1_ssd_lite_predictor(net, candidate_size=200,device_id=config.device_id)
elif net_type == 'mb2_ssd_lite':
    predictor = create_mobilenetv2_ssd_lite_predictor(net, candidate_size=200,device_id=config.device_id)
elif net_type == 'mb3_ssd_lite':
    predictor = create_mobilenetv3_ssd_lite_predictor(net, candidate_size=200, device_id=config.device_id)
elif net_type == 'sq_ssd_lite':
